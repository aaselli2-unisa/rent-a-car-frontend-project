...
const ShiftTypeTable: React.FC = () => {
  ...
  const changeRowsPerPage = (rowsPerPage: number, page: number) => { // Function to change row count
  ...
  const sort = (page: number, sortOrder: { name: string; direction: "asc" | "desc" }) => {
    setIsLoading(true);
    // Determine the sorting operation based on clicked column
  ...
  return (
    ...
    <h2 className="h2-card">SHIFT TYPE</h2>
    <div className="form">
      <MUIDataTable
        title={
